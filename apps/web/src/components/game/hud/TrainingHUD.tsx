...
function StatBar({ value, max = 1, color }: { value: number; max?: number; color: string }) {
  ...
  return (
    <div className="training-hud__stat-bar relative h-2.5 w-full overflow-hidden rounded-full bg-[var(--color-bg-deep)]">
      <motion.div
        className="training-hud__stat-bar-fill absolute inset-y-0 left-0 rounded-full"
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'training-hud training-hud--glass',
        ...
        >
          ...
          <div
            className={cn(
              'training-hud__header',
              'px-4 py-3.5 transition-colors',
              onToggle ? 'cursor-pointer hover:bg-white/5' : '',
            )}
            onClick={onToggle}
          >
            <div className="training-hud__header-row flex items-center justify-between">
              <div className="training-hud__title-group flex items-center gap-2.5">
                <div className="training-hud__status-dot h-2 w-2 rounded-full bg-[var(--color-success)] shadow-[0_0_8px_var(--color-success)]" />
                <span className="training-hud__title text-[13px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">
                  训练提示
                </span>
              </div>
              <span className="training-hud__icon text-[16px] text-[var(--color-text-dim)]">📊</span>
            </div>
            <div className="training-hud__divider mt-2 h-px bg-white/6" />
          </div>

          {/* Stats Body */}
          <div className="training-hud__body flex flex-col gap-3.5 px-4 pb-4">
            {/* 手牌强度 */}
            <div className="training-hud__metric training-hud__metric--strength flex flex-col gap-1.5">
            ...
            {/* 分隔 */}
            <div className="training-hud__divider h-px bg-white/5" />

            {/* 位置 */}
            {data?.position && (
              <div className="training-hud__position flex items-center justify-between">
                <span className="training-hud__position-label text-[12px] text-[var(--color-text-muted)]">位置</span>
                <span className="training-hud__position-badge rounded-md bg-[var(--color-bg-elevated)] px-2 py-1 text-[12px] font-semibold text-[var(--color-text-primary)]">
                  {data.position}
                </span>
              </div>
            )}
            ...
            {/* 建议 */}
            {suggestionCfg && (
              <div className={cn('training-hud__suggestion flex items-center gap-2 rounded-xl px-3 py-2.5', suggestionCfg.bg)}>
                <span className="training-hud__suggestion-icon">{suggestionCfg.icon}</span>
                <div className="training-hud__suggestion-content flex flex-col">
                  <span className={cn('training-hud__suggestion-title text-[13px] font-semibold', suggestionCfg.color)}>
                    {suggestionCfg.label}
                  </span>
                  {data?.suggestionReason && (
                    <span className="training-hud__suggestion-reason mt-0.5 text-[11px] leading-tight text-[var(--color-text-muted)]">
                      {data.suggestionReason}
                    </span>
                  )}
                </div>
