# AiPoker 玩法 PRD（Gameplay Spec）

> 版本：v1.1 | 日期：2026-03-03 | 范围：玩法与规则实现（不含 UI 细节、不含分布式扩展）
>
...
### 2.3 自由模式（Cash-like 练习桌）Feature
...
- `SB = Button = buttonMarkerSeat`（按钮位即小盲）
...
- `raiseTo(total)`（当 `currentBetToMatch > 0` 时）
- `all_in`（实现上可统一为 `raiseTo(streetCommitted + stack)` 或 `call(all-in)`，但 UI 可给单独按钮）
...
- 下注轮：
  - preflop：min-raise = BB（raiseTo=2BB 合法）
  - postflop：min-bet = BB
